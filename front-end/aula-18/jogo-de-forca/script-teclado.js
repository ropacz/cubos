...
const bloquarLetra = (letra) => {
    for(let i = 0; i < botoesLetras.length; i++){
        if( botoesLetras[i].innerText == letra){
            botoesLetras[i].disabled = true
            break
        }
    }
}
...
const exibeLetra = (palavra, letra) => {
    let resposta = false
    let camposSpan = document.querySelectorAll(".letras span")
    for (let i = 0; i < palavra.length; i++) {
        if (camposSpan[i].innerText === "" && palavra[i] === letra) {
